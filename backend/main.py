...
"""
GreenPulse Backend — Integrated Auth & AI Logic
"""
import os, json, math, random, time
from datetime import datetime
from typing import Optional
...
# ── Auth Data Store ─────────────────────────────────────────────────────────
users_db = {"demo@greenpulse.com": {"password": "password123", "name": "Green Explorer"}}
...
async def register(req: AuthRequest):
    ...
    users_db[req.email] = {"password": req.password, "name": req.name or "Eco Traveler"}
...
async def login(req: AuthRequest):
    ...
    return {"user": {"name": user["name"], "email": req.email}, "token": "gp_token_123"}

# ── Your Original Gemini/AQI Routes Below ───────────────────────────────────
# (Keep your @app.get("/api/dashboard"), plan_routes, etc., from your main.py)
